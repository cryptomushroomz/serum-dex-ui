...
export default function SelectCoinHeader({ selectedMarket, currency, markPrice }) {
  const [theme] = useContext(ThemeContext)
  const [last24hOrderPrice, setLast24hOrderPrice] = useState(null)
  const [callPriceHistoryTime, setCallPriceHistoryTime] = useState(moment.valueOf())
...
const PricePercentComponet = React.memo(({ orgPrice, newPrice }) => {
  ...
  return (
    <span style={{ marginLeft: '5px', marginRight: '5px', color: percentColor }}>
      {percent > 0 ? '+' : ''}
      {percent.toFixed(2)}%
    </span>
  )
...
const MarkPriceComponent = React.memo(
  ({ theme, markPrice, last24hOrderPrice, currency }) => {
    ...
    let formattedMarkPrice = numberWithCommas(
      markPrice && market?.tickSize && calCurrencyPrice(markPrice, currency.lowPrice).toFixed(tickSize),
      currency,
    )
    ...
    if (markPrice && last24hOrderPrice) {
      intervalPrice = numberWithCommas(
        calCurrencyPrice(markPrice - last24hOrderPrice, currency.lowPrice).toFixed(tickSize),
        currency,
      )
      if (Number(markPrice) !== Number(intervalPrice) && last24hOrderPrice < markPrice) {
        markPriceColor = '#26A69A'
      } else if (Number(markPrice) !== Number(intervalPrice) && last24hOrderPrice > markPrice) {
    ...
    return (
      <div>
        <div style={{ letterSpacing: '1px', marginTop: '20px', fontWeight: 'bold', lineHeight: '120%' }}>
          <span style={{ fontSize: '32px', color: markPriceColor }}>{`${formattedMarkPrice ?? ''}`}</span>
          <span style={{ color: markPriceColor, fontSize: '14px' }}>{selectQuoteCurrency ?? ''}</span>
        </div>
        {markPrice && last24hOrderPrice && Number(markPrice) !== Number(intervalPrice) ? (
          <div style={{ lineHeight: '120%', marginTop: '3px' }}>
            <span style={{ fontSize: '11px', color: '#666' }}>{trText('change_percent')}</span>
            <PricePercentComponet orgPrice={last24hOrderPrice} newPrice={markPrice} />
            <span style={{ color: markPriceColor, fontWeight: 'bold' }}>
              {last24hOrderPrice < markPrice && <ArrowUpOutlined style={{ marginRight: 5, fontSize: '15px' }} />}
              {last24hOrderPrice > markPrice && <ArrowDownOutlined style={{ marginRight: 5, fontSize: '15px' }} />}
              {intervalPrice}
            </span>
          </div>
        ) : null}
