...
export function TokenContextProvider(props: any) {
  ...
  const updateTokenAccounts = async (mints: Array<PublicKey | null | undefined>):Promise<void> => {
    ...
    for(const mint of mints) {
      ...
      if(mint.equals(SOL_MINT)) {
        ...
        provider.connection
          ...
          .then((acc: { lamports: number }) => {
            if (acc) {
              const solAcc = _OWNED_TOKEN_ACCOUNTS_CACHE.find(a => a.publicKey.equals(provider.wallet.publicKey))
              if(solAcc) {
    ...
    return getMultipleTokenAccounts(provider.connection, provider.wallet.publicKey, nonSolMints).then(
      (accs) => {
        if (accs) {
          accs.forEach(acc => {
